...
def _load_migration():
    """Parse migration file without importing alembic (broken markupsafe env)."""
    import pathlib
    import re
    ...
    obj = _Parsed()
    # Extract revision / down_revision via regex
    rev_match = re.search(r'^revision\s*=\s*["\']([^"\']+)["\']', src, re.MULTILINE)
    obj.revision = rev_match.group(1) if rev_match else None
    down_match = re.search(r'^down_revision\s*=\s*(None|["\'][^"\']+["\'])', src, re.MULTILINE)
    raw_down = down_match.group(1) if down_match else None
    obj.down_revision = None if raw_down == "None" else raw_down
    obj.has_upgrade = bool(re.search(r'^def upgrade', src, re.MULTILINE))
    obj.has_downgrade = bool(re.search(r'^def downgrade', src, re.MULTILINE))
    return obj
...
def test_migration_has_upgrade_and_downgrade():
    migration = _load_migration()
    assert migration.has_upgrade, "upgrade() function missing from migration"
    assert migration.has_downgrade, "downgrade() function missing from migration"
