...
function toSummary(patient: Patient): PatientSummary {
  ...
  const lastCaseDate =
    patientCases
      ...
      .sort((a, b) => b.localeCompare(a))[0] ?? patient.createdAt;
...
export async function getPatientDetailMock(
  patientId: string,
...
export async function updatePatientMock(
  patientId: string,
  request: Partial<CreatePatientRequest>,
): Promise<ApiResponse<Patient>> {
  const patient = patientsMock.find((item) => item.id === patientId);
  if (!patient) {
    return {
  ...
  if (request.fullName !== undefined) patient.fullName = request.fullName;
  if (request.dateOfBirth !== undefined)
    patient.dateOfBirth = request.dateOfBirth;
  if (request.gender !== undefined) patient.gender = request.gender;
  if (request.contact !== undefined) patient.contact = request.contact;
  if (request.medicalId !== undefined) patient.medicalId = request.medicalId;
