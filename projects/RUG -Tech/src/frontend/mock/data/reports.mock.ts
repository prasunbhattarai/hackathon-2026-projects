...
export const reportBundlesMock: ReportBundle[] = caseIdsWithAnalysis.slice(0, 20).map((caseId, index) => {
	...
	const urgency = analysis.severityLevel >= 4 ? "Urgent" : analysis.severityLevel === 3 ? "Priority" : "Routine";
	...
	return {
		...
		patient: {
			reportType: "patient",
			summary: "Your retinal scan was analyzed using an AI screening model and reviewed for disease risk.",
