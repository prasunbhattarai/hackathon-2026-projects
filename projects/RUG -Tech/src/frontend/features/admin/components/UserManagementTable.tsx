...
const roleBadgeVariant: Record<UserRole, 'info' | 'success' | 'none'> = {
  ...
  [UserRole.CLINIC_ADMIN]: 'info',
  [UserRole.DOCTOR]: 'success',
  [UserRole.LAB_ASSISTANT]: 'none',
}
    ...
    <div className={cn('flex flex-col gap-4', className)}>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        >
          ...
          {(['super_admin', 'clinic_admin', 'doctor', 'lab_assistant'] as UserRole[]).map((r) => (
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-[var(--border)]">
              {['Name', 'Email', 'Role', 'Clinic', 'Status', 'Created', 'Actions'].map((h) => (
                <th
                  key={h}
            ...
            {filteredUsers.map((user) => (
              ...
              >
                ...
                <td className="py-3 px-3 text-xs text-[var(--text-secondary)]">
                  {user.clinicId ? clinicMap.get(user.clinicId) ?? '—' : 'Platform'}
                </td>
                <td className="py-3 px-3">
