...
import { CaseStatus, type CaseRecord, type ImageQuality } from "@/types/case.types";

const CASE_STATUSES: CaseStatus[] = [
	...Array.from({ length: 5 }, () => CaseStatus.PENDING),
...
const submitterIds = usersMock
	.filter((user) => user.role === "doctor" || user.role === "lab_assistant")
	.map((user) => user.id);
...
export const casesMock: CaseRecord[] = Array.from({ length: 50 }, (_, index) => {
	...
	const score = clamp(Number((getBaseScore(status) + ((index % 11) - 5) * 0.045).toFixed(2)), 0.01, 0.99);
...
export const caseSummariesMock = casesToSummaries(casesMock, patientsMock).map((summary, index) => ({
	...summary,
